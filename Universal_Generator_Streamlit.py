...
def _convert_with_libreoffice(input_path, output_path):
    ...
    command = [libreoffice_cmd, "--headless", "--convert-to", "xlsx",
...
def format_worksheet_layout(ws):
    ...
    for col in ws.columns:
        ...
        for cell in col:
            ...
            line_len = max(len(l) for l in val.split("\n")) if "\n" in val else len(val)
            if line_len > max_len:
                max_len = line_len
        if max_len > 0:
            ws.column_dimensions[col_letter].width = min(max(max_len + 6, 18), 55)
...
def store_signature_excel(file_path, signature_b64, approver_name):
    ...
    ws.cell(row=1, column=1, value=crc_value)
    ws.cell(row=2, column=1, value=signature_b64)
    current_date = datetime.now().strftime("%d-%m-%Y")
    ws.cell(row=3, column=1, value=f"Approved by {approver_name} on {current_date}")
    try:
        format_worksheet_layout(ws)
    except Exception:
        pass
...
st.set_page_config(page_title="Digital Signature Generator", page_icon="pen", layout="centered")
...
if st.button("Generate Key Pair"):
    try:
        ...
        st.success("Keys generated! Download them as a ZIP below.")
                    ...
                    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                        for uploaded_doc in uploaded_batch_docs:
                            ...
                            try:
                                ...
                                output_name = os.path.splitext(filename)[0] + "_Signed" + out_ext
                                zf.writestr(output_name, signed_bytes)
                                success_count += 1
                    ...
                    if success_count > 0:
                        ...
                        if error_messages:
                            ...
                            for msg in error_messages:
                                st.write(f"- {msg}")
                        st.download_button(
                            f"Download Signed_by_{name_part}.zip",
                            zip_buffer.getvalue(),
                            f"Signed_by_{name_part}.zip",
